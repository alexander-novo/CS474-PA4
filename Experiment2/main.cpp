		...
		for (int j = 0; j < image.cols; j++){
			...
			for (int k = -1; k < 2; k++){
				for (int l = -1; l < 2; l++){
					...
					if(i + k < 0 || i + k >= image.rows || j + l < 0 || j + l >= image.cols){
   						output_pixel_value += 0;
   					}
   					//calculate output value
   					else
   						output_pixel_value += image[i + k][j + l] * sobel_mask[k + mask_size/2][l + mask_size/2];
...
void freq_filter(Image& image) {
	...
	for (int i = 0; i < 3; i++){
		for (int j = 0; j < 3; j++){
			...
			mask[x][y] = sobel_mask[i][j] * pow((-1), x + y); // center spectrum of mask		
		}
	}
	...
	for (int i = 0; i < image.rows; i++){
		for (int j = 0; j < image.cols; j++){
			...
			mask_transform[index] = std::complex<float>(0, mask_transform[index].imag()) * std::complex<float>(pow(-1, i + j));
	...
	for (int i = 0; i < image.rows; i++){
		for (int j = 0; j < image.cols; j++){
			...
			pixels[i*image.cols + j] = transform[i*image.cols + j].real() * pow(-1, i + j);	// undo spectrum centering	
		}
	}
...
int main(int argc, char** argv) {
	...
	std::ifstream inFile(argv[1]);
 	...
 	freq_filter(freq_filter_image);

	// Save output images
	std::ofstream outFile;

	outFile.open(std::string(argv[2]).substr(0, std::string(argv[2]).find('.')) + "_spatial.pgm");
	outFile << image;
	outFile.close();

	outFile.open(std::string(argv[2]).substr(0, std::string(argv[2]).find('.')) + "_frequency.pgm");
